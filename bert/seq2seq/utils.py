import torch
from bert.seq2seq.seq2seq_model import Seq2SeqModel
from bert.seq2seq.bert_cls_classifier import BertClsClassifier
from bert.seq2seq.bert_seq_labeling import BertSeqLabeling
from bert.seq2seq.bert_seq_labeling_crf import BertSeqLabelingCRF
from bert.seq2seq.bert_relation_extraction import BertRelationExtrac
...
def load_bert(vocab_path, model_name="roberta", model_class="seq2seq", target_size=0):
    ...
    if model_class == "seq2seq":
        ...
        bert_model = BertClsClassifier(
            vocab_path, target_size, model_name=model_name)
        return bert_model
    elif model_class == "sequence_labeling":
        # 序列标注模型
        if target_size == 0:
            raise Exception("必须传入参数 target_size，才能确定预测多少分类")
        bert_model = BertSeqLabeling(
            vocab_path, target_size, model_name=model_name)
        return bert_model
